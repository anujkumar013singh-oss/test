...
const AvailabilityBadge = () => (
  <motion.div
    ...
    className="absolute z-10 left-1/2 -translate-x-1/2 hidden md:flex items-center gap-2.5 pointer-events-none"
    style={{ top: "2.25rem" }}
  >
    ...
    <span className="bricolage-grotesque font-black text-sm tracking-[0.25em] uppercase text-white">
      Available for work
    </span>
  </motion.div>
);
...
const SocialStrip = () => {
  const socials = [
    ...
    { label: "LinkedIn", href: "https://www.linkedin.com/in/anuj-singh-developer" },
  ...
  return (
    <motion.div
      ...
      style={{ right: "64px", top: "112px", bottom: "194px", justifyContent: "center", gap: "1.5rem" }}
    >
      <span className="w-[1px] h-8 bg-white/40 flex-shrink-0" />
      {socials.map(({ label, href }) => (
        ...
        >
          <span className="bricolage-grotesque font-black text-sm tracking-[0.25em] uppercase text-white hover:text-white/80 group-hover:opacity-100 transition-opacity duration-300">
            {label}
          </span>
        </a>
      ))}
...
const SpinningCTA = () => (
  ...
  >
    ...
    `}</style>
    <a href="#contact" className="cta-wrap group relative flex items-center justify-center w-[155px] h-[155px]" aria-label="Get in touch">
      <svg viewBox="0 0 145 145" className="absolute inset-0 w-full h-full pointer-events-none">
        <circle cx="72.5" cy="72.5" r="69.5" fill="none" stroke="rgba(255,255,255,0.7)" strokeWidth="2" />
      </svg>
      <svg viewBox="0 0 145 145" className="cta-ring absolute inset-0 w-full h-full pointer-events-none">
        <defs>
          <path id="cta-circle-path" d="M72.5,72.5 m-55,0 a55,55 0 1,1 110,0 a55,55 0 1,1 -110,0" />
        </defs>
        <text fill="rgba(255,255,255,1)" fontSize="10.5" fontFamily="'Inter', sans-serif" fontWeight="900" letterSpacing="4">
      ...
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="relative z-10 w-8 h-8 text-white group-hover:text-black" style={{ transition: "color 0.3s ease" }}>
...
const MobileSocialStrip = () => {
  const socials = [
    { label: "Github", icon: Github, href: "https://github.com/anujkumar013singh-oss" },
    { label: "LinkedIn", icon: Linkedin, href: "https://www.linkedin.com/in/anuj-singh-developer" },
    { label: "Email", icon: "email" as const, href: "mailto:alonesurvivor03@gamil.com" },
    { label: "WhatsApp", icon: "whatsapp" as const, href: "https://wa.me/9654673316?text=Hi%20I%20saw%20your%20portfolio%20and%20want%20to%20connect" },
  ];
  return (
    <motion.div
      initial={{ opacity: 0, x: 20 }}
      ...
      {socials.map(({ label, icon: Icon, href }) => (
        <a key={label} href={href} target={href.startsWith("mailto") ? "_self" : "_blank"} rel="noopener noreferrer"
          ...
          {Icon === "whatsapp" ? (
            ...
            <svg viewBox="0 0 24 24" fill="currentColor" width={22} height={22}>
              <path d="M20 4H4C2.895 4 2 4.895 2 6V18C2 19.105 2.895 20 4 20H20C21.105 20 22 19.105 22 18V6C22 4.895 21.105 4 20 4ZM19.2 6L12 11.4L4.8 6H19.2ZM4 18V7.5L12 13.5L20 7.5V18H4Z"/>
            </svg>
          ) : (
